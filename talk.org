...
#+BEAMER_HEADER: \subtitle{UQ Pure Maths Seminar\\Joint with Mohammad Ivaki and Julian Scheuer}
#+AUTHOR: Paul Bryan
...
- Let \(M_0\) be a closed, unstable, minimal surface in \(N\).
  - If \(N\) has \(\ric \geq K_0 > 0\), Pitts showed closed, minimal surfaces exist up to \(n=6\). Schoen-Yau extended it to \(n=7\).
  - In this case, all minimal surfaces are unstable.
- Linearise the Flow:
  \[
  \partial_t u = \Box u - (\ric(\nu_0) + \abs{A_0}^2) u
  \]
  for normal graphs over \(M_0\)
- Note if \(u \equiv c\), then the energy \(I(u) = \int \abs{\nabla u}^2 - (\ric(\nu_0) + \abs{A_0}^2) < 0\)
- Therefore we have negative eigenvalues. Index: \(I =\) number of negative eigenvalues.
- Unstable manifold theorem
  - There is an \(I\) dimensional linear-manifold of smooth, ancient solutions such that \(\lim_{t\to-\infty} M_t = M_0\) smoothly.
- Principle eigenvalue is simple (at least for MCF): corresponding eigenspace is one-dimensional. (Harnack inequality!)
- Corresponds to *unique* monotone motion - for all \(t_0\), \(M_{t}\) lies on one side of \(M_{t_0}\) for all \(t > t_0\).
** Convex Ancient Solutions are Rare!
For a convex ancient solution of MCF in locally symmetric, non-negative Einstein background
- Bonus term in the Harnack:
  \[
  \partial_t H - \abs{\nabla H}_h^2 + \frac{1}{2} \frac{H}{t-t_0} - \frac{\bar{R}}{n+1} H\geq 0.
  \]
- Sending \(t_0 \to -\infty\)
  \[
  \partial_t H \geq \frac{\bar{R}}{n+1} H, \quad (\bar{R} > 0).
  \]
- Therefore
  \[
  H \leq e^{\tfrac{\bar{R}}{n+1} t} \to 0, \quad t \to -\infty
  \]
- For convex flows \(0 \leq \abs{A}^2 \leq H^2\).
- Higher regularity \(\Rightarrow\) \(\abs{A}^2 \to 0\) in \(C^{\infty}\) hence \(M_t\) converges smoothly to a smooth, closed totally geodesic \(M_0\).
- *But* \(\exists M_0\) smooth, closed totally geodesic hypersurface \(\Rightarrow N\) is a the sphere! Eg, \(\mathbb{CP}^n\) has no such hypersurfaces.
- Therefore, no convex ancient solutions to MCF exist!
** Degree One Homogeneity in the sphere
- Speed \(F\) is convex, degree one homogeneous.
  \[
  \frac{\kappa_{\min}}{F} \leq \frac{\kappa_{\min}}{H} \leq \frac{1}{n}.
  \]
- Evolution (compare Bonus term in Harnack!):
  \[
  (\partial_t - \Box) \left(\frac{\kappa_{\min}}{F} - \frac{1}{n}\right) \geq - 2n \left(\frac{\kappa_{\min}}{F} - \frac{1}{n}\right)
  \]
- Max Principle (for ancient solutions):
  \[
  \frac{\kappa_{\min}}{F} \geq \frac{1}{n}
  \]
- Therefore
  \[
  \frac{1}{n} \leq \frac{\kappa_{\min}}{H} \leq \frac{1}{n} \Rightarrow H \equiv n \kappa_{\min}
  \]
- That is, \(M_t\) is closed, umbillic hypersurface \(\Rightarrow\) \(M_t\) is a sphere of latitude.
** Arbitrary Flows in the Sphere
- Take *any* parabolic, geometric flow of closed hypersurfaces in the sphere.
- Assume \(M_t\) is a convex, ancient solution with \(H\) bounded on \((-\infty, -1)\).
- Then \(M_t\) is a sphere of latitude.
- Proof:
  - \(H\) bound implies uniform (in time) interior sphere condition.
  - We obtain a backwards limit \(M_0\) with interior sphere condition.
  - do Carmo-Warner \(\Rightarrow\) \(M_0\) is either an equator or strictly contained in a hemisphere.
  - Strictly contained in a hemisphere cannot be ancient because of latitude sphere barriers!
  - Aleksandrov reflection shows this symmetry is preserved.
